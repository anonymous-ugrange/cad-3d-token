...
MODEL_NAME_DICT={"bert_large_uncased":"bert-large-uncased"}
...
class TextEmbedder(nn.Module):
    def __init__(self, model_name:str, cache_dir:str, max_seq_len:int):
        ...
        self.tokenizer = BertTokenizer.from_pretrained("/".join([cache_dir, self.model_name]))
        self.model = BertModel.from_pretrained(
                "/".join([cache_dir, self.model_name]), max_position_embeddings=max_seq_len
            ).to(device)
