...
class TokenController:
    def __init__(self, num_texts, device):
        self.counter = torch.zeros((num_texts, 1), dtype=torch.long, device=device)
...
class CADDecoder(nn.Module):
    ...
    def forward(
        ...
        Sx = self.seq_output_x(S).unsqueeze(dim=2)  # (B,N1,1,one_hot_size)
    ...
    def decode(
        ...
        if not ORIGINAL:
            token_controller = TokenController(num_texts, device)
