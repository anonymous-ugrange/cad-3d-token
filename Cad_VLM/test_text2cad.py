...
from Cad_VLM.dataprep.t2c_dataset_new import get_dataloaders, StartEnd
...
def main():
    ...
    for checkpoint_file in checkpoint_files:
        check_file_with_sleep(checkpoint_file, sleep_interval=1200)
            ...
            for uid_level, vec_dict, prompt, _ in pbar:
                ...
                for topk_index in range(1, TOPK + 1):
                    ...
                    for i in range(vec_dict["cad_vec"].shape[0]):
                        ...
                        pred_cad_vec = StartEnd.restore_vec(pred_cad_seq_dict["cad_vec"][i])
                        # if topk_index == 1:
                        ...
                        if is_invalid == 0:
                            ...
                            if config["test"]["clouds"] and cd > 50 and \
                                any(k in level for k in ["expert"]):
                                save_dir = os.path.join(log_dir, "clouds", 
                                                        f"{uid}", f"{level}_cd{cd}")
