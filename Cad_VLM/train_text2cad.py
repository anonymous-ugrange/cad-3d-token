...
from Cad_VLM.dataprep.t2c_dataset_new import get_dataloaders
...
# >>> DDP CHANGE: imports for distributed training
...
# >>> DDP CHANGE: setup / cleanup for torch.distributed
def setup_distributed(rank, world_size):
    # Use environment init (compatible with torchrun)
...
def main():
    ...
    # If launched under torchrun, LOCAL_RANK will be present. Otherwise fallback to spawn.
    if "LOCAL_RANK" in os.environ:
        # torchrun style: single process per GPU
        world_size = int(os.environ.get("WORLD_SIZE", ngpus))
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
        run(local_rank, world_size, config, args)
    else:
        # Fallback: spawn processes (useful if user runs `python train.py` directly)
        world_size = ngpus
        mp.spawn(run, args=(world_size, config, args), nprocs=world_size, join=True)


# >>> DDP CHANGE: run() creates model, wraps with DDP, and calls train_model
def run(rank, world_size, config, args):
    ...
    for param in model.base_text_embedder.parameters():
        param.requires_grad = False

    # Wrap with DDP (each process works on a single GPU)
    ...
    # lr = min(0.001, lr * (batch * world_size) / 16)
    t2clogger.info(f"learning rate adjust to {lr}")
    ...
    criterions = [CELoss(device=device),
    ...
    # Only rank 0 creates dirs and saves config
...
def train_model(
    ...
    checkpoint_only_model_file = os.path.join(log_dir, f"{checkpoint_name}_model.pth")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for _, vec_dict, prompt, mask_cad_dict in data_iterator:
            step += 1
...
def validation_one_epoch(
    ...
    for _, vec_dict, prompt, _ in data_iterator:
